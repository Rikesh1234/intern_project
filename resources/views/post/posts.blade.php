...
<body class="hold-transition sidebar-mini layout-fixed">
      ...
      <li class="nav-item">
        ...
        <div class="content-wrapper">
            ...
            <section class="content">
                <div class="container-fluid">
                    @can('create', \App\Models\postModel::class)
                        <div class="d-flex flex-row-reverse">
                            <a href="{{ route('cms.post.create') }}">
                                <div class="p-2 mb-3 bg-success text-dark shadow-sm rounded" style="cursor: pointer;"> Add
                                    New Post &nbsp; <i class="fa fa-plus" aria-hidden="true"></i>
                                </div>
                            </a>
                        </div>
                    @endcan
                    ...
                    <table class="table">
                        <thead class="thead-dark">
                            <tr>
                                ...
                                @canany(['update', 'delete'], \App\Models\postModel::class)
                            ...
                            <tbody>
                                @foreach ($posts as $post)
                                    <tr id="postRow_{{ $post->id }}">
                                        ...
                                        <td>{!! Str::limit(strip_tags($post->description), 40, '...') !!}</td>
                                        <td>{{ Str::limit($post->summary, 40, '...') }}</td>
                                        <td>
                                            <input type="checkbox" data-post-id="{{ $post->id }}"
                                                {{ $post->status ? 'checked' : '' }} data-toggle="toggle"
                                                name="status" data-onstyle="success" data-offstyle="danger"
                                                data-on="Active" data-off="Inactive">
                                        </td>
                                        <td><img src="{{ $post->image ? asset('uploads/posts/' . $post->image) : asset('uploads/posts/default.jpg') }}"
                                                height="50px" width="50px"></td>
                                        <td class="d-flex justify-center " style="align-items: center; gap:8px;">
                                            @can('update', \App\Models\postModel::class)
                                                <a href="{{ route('cms.post.edit', $post->id) }}"><span
                                                        class="p-2 mb-3 bg-primary text-dark shadow-sm rounded"
                                                        style="cursor: pointer;"> Edit &nbsp; <i
                                                            class="fas fa-edit"></i></span></a>
                                            @endcan
                                            @can('delete', \App\Models\postModel::class)
                    ...
                    <div class="pagination d-flex justify-content-between flex-row"><div class="pagination-show">
                            Showing <span id="first-data">{{$posts->firstItem()}}</span> to <span id="last-data">{{ $posts->lastItem() }}</span> of <span id="total-data">{{ $posts->total() }}</span>
                    </div>
                        {{ $posts->links() }}
                    </div>
    ...
    <script>
        function confirmDelete(postId) {
            ...
            if (confirmation) {
                axios.delete('{{ route('cms.post.destroy', ['id' => '__postId']) }}'.replace('__postId', postId))
                    .then(response => {
                        showDeleteAlert(response.data.success, 'alert-success');
                        $("#postRow_" + postId).remove();
                    })
                    .catch(error => {
    ...
    <script>
        $(function() {
            $('input[name="status"]').change(function() {
                ...
                axios.patch(`/cms/post/update-status/${postId}/${status}`)
                    .then(response => {
