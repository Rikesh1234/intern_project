...
<body class="hold-transition sidebar-mini layout-fixed">
      ...
      <li class="nav-item">
        ...
        <div class="content-wrapper">
            ...
            <section class="content">
                <div class="container-fluid">
                    @can('create', \App\Models\categories::class)
                        <div class="d-flex flex-row-reverse">
                            <a href="{{ route('cms.categories.create') }}">
                                <div class="p-2 mb-3 bg-success text-dark shadow-sm rounded" style="cursor: pointer;"> Add
                                    New Categories &nbsp; <i class="fa fa-plus" aria-hidden="true"></i>
                                </div>
                            </a>
                        </div>
                    @endcan
                    ...
                    <table class="table">
                        <thead class="thead-dark">
                            <tr>
                                ...
                                @canany(['update', 'delete'], \App\Models\categories::class)
                        ...
                        @if ($categories)
                            <tbody>
                                @foreach ($categories as $cat)
                                    <tr id="categoriesRow_{{ $cat->id }}">
                                        <td>{{ $cat->title }}</td>
                                        <td>{{ $cat->slug }}</td>
                                        <td style="white-space: normal;">{!! Str::limit(strip_tags($cat->description), 40, '...') !!}</td>
                                        <td>
                                            {!! Form::checkbox('status', 1, $cat->status, [
                                                'data-categories-id' => $cat->id,
                                                'data-toggle' => 'toggle',
                                        ...
                                        <td class="d-flex justify-center " style="align-items: center; gap:8px;">
                                            @can('update', \App\Models\categories::class)
                                                <a href="{{ route('cms.categories.edit', $cat) }}"><span
                                                        class="p-2 mb-3 bg-primary text-dark shadow-sm rounded"
                                                        style="cursor: pointer;"> Edit &nbsp; <i
                                                            class="fas fa-edit"></i></span></a>
                                            @endcan
                                            @can('delete', \App\Models\categories::class)
                                                <a href="#" onclick="confirmDelete({{ $cat->id }})">
    ...
    <script>
        function confirmDelete(categoriesId) {
            var confirmation = confirm("Are you sure you want to delete this categories?");
            if (confirmation) {
                axios.delete('{{ route('cms.categories.destroy', ['id' => '__postId']) }}'.replace('__postId',
                        categoriesId))
                    .then(response => {
                        showDeleteAlert(response.data.success, 'alert-success');
                        $("#categoriesRow_" + categoriesId).remove();
                    })
                    .catch(error => {
    ...
    <script>
        $(function() {
            $('input[name="status"]').change(function() {
                ...
                axios.patch(`/cms/categories/update-status/${categoriesId}/${status}`)
                    .then(response => {
