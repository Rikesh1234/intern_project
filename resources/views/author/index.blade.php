    ...
    <section class="content">
      <div class="container-fluid">
        ...
        <div class="d-flex flex-row-reverse">
            <a href="{{route('cms.author.create')}}"><div class="p-2 mb-3 bg-success text-dark shadow-sm rounded" style="cursor: pointer;"> Add New Author &nbsp; <i class="fa fa-plus" aria-hidden="true"></i>
            </div></a>
        </div>
        @endcan
        ...
        <table class="table">
            <thead class="thead-dark">
              <tr>
                ...
                @canany(['update', 'delete'], \App\Models\author::class)
            ...
            <tbody>
              ...
              <tr id="authorRow_{{ $authors->id }}">
                <td>{{$authors->name}}</td>
                <td>{!! Str::limit($authors->description, 40, '...') !!}</td>
                <td>
                  {!! Form::checkbox('status', 1, $authors->status, [
                      'data-author-id' => $authors->id,
                      'data-toggle' => 'toggle',
                ...
                <td><img src="{{ $authors->image ? asset('uploads/authors/'.$authors->image) : asset('uploads/authors/default.jpeg')  }}" height="50px" width="50px"></td>
                <td class="d-flex justify-center " style="align-items: center; gap:8px;">
                    @can("update",\App\Models\author::class)
                  <a href="{{route('cms.author.edit',$authors)}}"><span class="p-2 mb-3 bg-primary text-dark shadow-sm rounded" style="cursor: pointer;"> Edit &nbsp; <i class="fas fa-edit"></i></span></a>
                  @endcan
                  @can("delete",\App\Models\author::class)
                  <a href="#" onclick="confirmDelete({{ $authors->id }})">
...
<script>
  function confirmDelete(authorId) {
      ...
      if (confirmation) {
          axios.delete('{{ route("cms.author.destroy", ["id" => "__postId"]) }}'.replace('__postId', authorId))
              .then(response => {
                showDeleteAlert(response.data.success, 'alert-success');
                $("#authorRow_" + authorId).remove();
              })
              .catch(error => {
...
<script>
 $(function() {
    $('input[name="status"]').change(function() {
        var postId = $(this).data('author-id');
        var status = $(this).prop('checked') ? 1 : 0;

        axios.patch(`/cms/author/update-status/${postId}/${status}`)
            .then(response => {
